 ...
 * used to live as inline buttons (Cancel, Log, Edit, Download). Rerun
 * and Clear stay inline as the two highest-frequency mutations.
 *
...
interface Props {
   ...
   *  removed), the menu still renders Cancel/Log/Edit/Download but
   *  hides the device section since those actions don't have a target
   *  to act on. */
  target: Target | null;
  inProgress: boolean;
  hasLog: boolean;
    ...
    <DropdownMenu open={open} onOpenChange={onOpenChange}>
      ...
      >
        <DropdownMenuGroup>
          ...
          {hasLog && (
            <DropdownMenuItem onClick={() => onOpenLog(job.id)}>
              View log
            </DropdownMenuItem>
          )}
...
function propsEqual(prev: Props, next: Props): boolean {
  ...
  if (prev.hasLog !== next.hasLog) return false;
