...
- **10 GiB default**, configurable in **Settings → Disk management → Worker disk quota — fleet default**. Per-worker overrides via **Set disk quota…** in the Workers-tab hamburger; the Connect Worker dialog has a matching radio so a fresh worker can be sized differently from the start.
...
- The Workers tab now shows `Quota: 2.1 / 10 GiB` per worker (yellow over 80 %, red over 95 %).
