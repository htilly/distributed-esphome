...
- **`privileged: [NET_RAW]`** — needed by the per-device **Ping** diagnostic on installs where unprivileged ICMP is disabled (HAOS ships `net.ipv4.ping_group_range = 1 0`, an empty range). The ping helper tries unprivileged datagram ICMP first and only falls back to a raw socket when the kernel refuses the safer path. Scoped to ICMP — the endpoint accepts no arbitrary host, only the configured device's resolved OTA address. This is the only non-default Linux capability the add-on holds.
