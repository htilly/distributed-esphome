...
- [ ] 23 SU items -  let's add a tab that shows the schedule in a tabular format, Including a column that shows next execution, a column for the device, a column for the version, and a column for which worker it's going to run on. Basically all of the options that we are showing in the modal.
