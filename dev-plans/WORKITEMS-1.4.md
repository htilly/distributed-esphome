...
- [ ]  31 In the upgrade dialog for scheduled recurring tasks, we need to allow specifying a "latest" option that doesn't specify one particular version. It should just specify whatever is the currently installed, which is whatever we've identified as the installed version. 
