...
| F-02 | Worker auto-update executes arbitrary server code    | High     | FIXED (1.4.1, SC.4) | Source-code auto-update removed. `/api/v1/client/code` returns 410 Gone; server no longer advertises `server_client_version` on heartbeat responses; client's `_apply_update` / `_update_available` / `_initial_version_check` are stubbed out. Workers now update by rebuilding the Docker image (`docker pull` + restart). The arbitrary-Python attack surface is gone. |
