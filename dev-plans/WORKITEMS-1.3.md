...
- [ ] **#179** — (GitHub #2, follow-up to #159) Thread-only and static-IP devices produce duplicate rows. Two root causes, both confirmed by reading the user's `generic-thread.yaml` (pure `openthread:` block, no `wifi:`/`ethernet:`) against ESPHome upstream:

  1. `scanner.build_name_to_target_map` (scanner.py:306-359) only reads `wifi.use_address`. It ignores `ethernet.use_address`, `openthread.use_address`, and `{wifi,ethernet}.manual_ip.static_ip`. So Thread-only devices and statically-IP'd devices have no `address_override` registered, no proactive `Device` row is created from the YAML side, and any mDNS-discovered entry lands in a separate row from the YAML-derived target.

  2. `device_poller._handle_service_change` parses only IPv4 from zeroconf (`socket.inet_ntoa(info.addresses[0])` at device_poller.py:178-181). Thread devices advertise via SRP/mDNS with **IPv6 AAAA records** (16-byte addresses). `inet_ntoa` either errors or silently mis-parses, so the device either fails to register or registers with a garbled address — the YAML row and the mDNS row never reconcile, doubling the row.

  **Fix:** Mirror ESPHome's canonical address resolver `esphome.core.CORE.address` (`esphome/core/__init__.py` ~L660), which checks `wifi → ethernet → openthread` in order, and for each block honors `use_address` → `manual_ip.static_ip` → `<name>.local`. Add a single helper `get_device_address(config, device_name)` in `scanner.py` and call it for every target so `address_overrides` is populated for every YAML, not just wifi-with-explicit-use_address. In `device_poller.py`, branch on `len(addr) == 16` and use `socket.inet_ntop(AF_INET6, addr)` for IPv6, prefer IPv4 when both are present, and merge mDNS-discovered entries into YAML-derived ones by normalized `esphome.name` rather than inserting a new row. For Thread-only targets (`openthread:` present, no `wifi:`/`ethernet:`), the YAML-derived row is authoritative.

  **Tests:** unit tests for `get_device_address` covering each network type × `use_address`/`manual_ip.static_ip`/default fallback (9 cases), plus an integration test with a Thread-only fixture YAML asserting one row, not two.
