...
- `dev-plans/WORKITEMS-1.7.0.md` — current release (Honest Gold + heffneil-inspired device-management polish — the tier-flip 1.6.2 pivoted away from, plus four user-facing features lifted from `heffneil/esphome-enhanced-dashboard`: sidecar-JSON tagging, mark-inactive, ping diagnostic, install-to-specific-address. Renamed from 1.6.3 after the heffneil scope crossed the patch/minor line).
- `dev-plans/WORKITEMS-1.8.md` — next release (LLM assistance — multi-provider in-editor YAML assistant, fleet-wide tool-calling chat, release breaking-change analyzer).
- `dev-plans/WORKITEMS-1.9.md` — after-next release (ESPHome dashboard parity — serial flashing, log tail after OTA, adopt discovered devices, remote compilation for cloud build workers).
